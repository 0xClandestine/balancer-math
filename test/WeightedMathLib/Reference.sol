...
library WeightedMathLib {
    ...
    function _calculateInvariant(
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < normalizedWeights.length; i++) {
                invariant =
                    invariant.mulDown(balances[i].powDown(normalizedWeights[i]));
            }
    ...
    function _calcOutGivenIn(
        ...
        _require(
            amountIn <= balanceIn.mulDown(_MAX_IN_RATIO), Errors.MAX_IN_RATIO
        );
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < balances.length; i++) {
                balanceRatiosWithFee[i] =
                    balances[i].add(amountsIn[i]).divDown(balances[i]);
    ...
    function _calcBptOutGivenExactTokenIn(
        ...
        unchecked {
            ...
            {
                uint256 balanceRatioWithFee =
                    balance.add(amountIn).divDown(balance);
                ...
                uint256 invariantRatioWithFees = balanceRatioWithFee.mulDown(
                    normalizedWeight
                ...
                if (balanceRatioWithFee > invariantRatioWithFees) {
                    ...
                    amountInWithoutFee =
                        nonTaxableAmount.add(taxableAmount.sub(swapFee));
                } else {
            ...
            uint256 balanceRatio =
                balance.add(amountInWithoutFee).divDown(balance);
    ...
    function _calcTokenInGivenExactBptOut(
        ...
        uint256 taxableAmount =
            amountInWithoutFee.mulUp(normalizedWeight.complement());
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < balances.length; i++) {
                balanceRatiosWithoutFee[i] =
                    balances[i].sub(amountsOut[i]).divUp(balances[i]);
    ...
    function _calcBptInGivenExactTokenOut(
        ...
        uint256 invariantRatioWithoutFees = balanceRatioWithoutFee.mulUp(
            normalizedWeight
        ).add(normalizedWeight.complement());
        ...
        if (invariantRatioWithoutFees > balanceRatioWithoutFee) {
            ...
            uint256 nonTaxableAmount =
                balance.mulDown(invariantRatioWithoutFees.complement());
    ...
    function _calcTokenOutGivenExactBptIn(
        ...
        uint256 taxableAmount =
            amountOutWithoutFee.mulUp(normalizedWeight.complement());
        uint256 nonTaxableAmount = amountOutWithoutFee.sub(taxableAmount);
        uint256 taxableAmountMinusFees =
            taxableAmount.mulUp(swapFeePercentage.complement());
        ...
        returns (uint256)
    {
        ...
        uint256 weightSumRatio =
            FixedPoint.ONE.divDown(FixedPoint.ONE.sub(normalizedWeight));
        ...
        returns (uint256 result)
    {
        unchecked {
            ...
            assembly {
                result :=
                    mul(
                        iszero(iszero(aInflated)), add(div(sub(aInflated, 1), b), 1)
                    )
            }
        }
    }
...
library Math {
    ...
    function add(int256 a, int256 b) internal pure returns (int256) {
        unchecked {
            int256 c = a + b;
            _require(
                (b >= 0 && c >= a) || (b < 0 && c < a), Errors.ADD_OVERFLOW
            );
            return c;
        }
    }
    ...
    function sub(int256 a, int256 b) internal pure returns (int256) {
        unchecked {
            int256 c = a - b;
            _require(
                (b >= 0 && c <= a) || (b < 0 && c > a), Errors.SUB_OVERFLOW
            );
            return c;
        }
    }
    ...
    function div(uint256 a, uint256 b, bool roundUp)
        internal
        pure
        returns (uint256)
    {
