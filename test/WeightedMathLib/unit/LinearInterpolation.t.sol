...
contract LinearInterpolationTest is Test {
    ...
    function testValueCannotDecreaseMoreThanDelta() public {
        ...
        assertEq(
            WeightedMathLib.linearInterpolation({
                x: 1_000 ether,
                y: 100 ether,
                i: 11,
                n: 10
    ...
    function testValueCannotDecreaseMoreThanDelta(uint128 i, uint128 n)
        public
    {
        vm.assume(i > n);
        vm.assume(n > 0);
    ...
    /// @dev Using uint128 for fuzzing to avoid i*n overflow.
    /// type(uint128).max**2 == type(uint256).max
    function testValueCannotIncreaseMoreThanDelta(uint128 i, uint128 n)
        public
    {
        vm.assume(i > n);
        vm.assume(n > 0);
